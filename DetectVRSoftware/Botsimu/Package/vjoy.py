...
CONST_DLL_VJOY = os.path.dirname(os.path.realpath(__file__)) + "\\vJoyInterface.dll"
HOST = "localhost"
PORT = 9988
...
def walk():
    vj.open()
    joystickPosition = vj.generateJoystickPosition(wAxisY = 8195)
    vj.update(joystickPosition)
    vj.close()

def stop():
    vj.open()
    joystickPosition = vj.generateJoystickPosition(wAxisY = 16393)
    vj.update(joystickPosition)
    vj.close()
